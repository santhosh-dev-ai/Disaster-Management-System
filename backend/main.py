"""
Intelligent Disaster Management System - Main Application
"""
...
app = FastAPI(
    ...
    description="AI-powered disaster monitoring, prediction, and response platform",
    version="1.0.0",
    docs_url="/docs",
    redoc_url="/redoc",
)
...
@app.get("/")
async def root():
    from database import is_fallback_mode
    return {
        "name": "Intelligent Disaster Management System",
        "version": "1.0.0",
        "status": "operational",
        "database": "In-Memory (fallback)" if is_fallback_mode() else "Supabase (PostgreSQL)",
        "docs": "/docs",
    }
...
async def health_check():
    ...
    return {
        "status": "healthy",
        "service": "disaster-mgmt-api",
        "database": "in-memory-fallback" if is_fallback_mode() else "supabase",
    }
...
async def websocket_user_alerts(websocket: WebSocket, user_id: int):
    """WebSocket endpoint for user-specific alert updates."""
    await manager.connect(websocket, user_id)
    try:
        while True:
            data = await websocket.receive_text()
            await manager.broadcast({"type": "message", "data": data, "user_id": user_id})
    except WebSocketDisconnect:
        manager.disconnect(websocket, user_id)


# ─── Seed Fallback Data ────────────────────────────────────
def _seed_fallback_data() -> None:
    """Seed the in-memory store with default data for development."""
    from database import supabase, is_fallback_mode
    ...
    alerts = [
        {
            ...
            "description": "Heavy monsoon rainfall causing severe flooding in low-lying areas.",
            "disaster_type": "flood",
            "severity": "critical",
            "latitude": 19.0760,
            "longitude": 72.8777,
        ...
        },
        {
            ...
            "description": "Category 3 hurricane approaching. Evacuation orders issued.",
        ...
        },
        {
            ...
            "description": "Tsunami watch issued following undersea earthquake.",
        ...
        },
        {
            ...
            "description": "Flash flood warning in effect. Avoid low-lying areas.",
            "disaster_type": "flood",
            "severity": "high",
            "latitude": 29.7604,
            "longitude": -95.3698,
    ...
    # ── Resources ──
    resources = [
        {"name": "Emergency Medical Team Alpha", "resource_type": "Medical", "description": "20-person medical response team", "quantity": 1, "unit": "team", "status": "available", "latitude": 38.9072, "longitude": -77.0369, "location_name": "Washington DC HQ"},
        {"name": "Water Purification Units", "resource_type": "Water", "description": "Portable water purification systems", "quantity": 15, "unit": "units", "status": "available", "latitude": 40.7128, "longitude": -74.0060, "location_name": "New York Warehouse"},
        {"name": "Emergency Shelters", "resource_type": "Shelter", "description": "Pop-up emergency shelters, capacity 50 each", "quantity": 30, "unit": "shelters", "status": "available", "latitude": 34.0522, "longitude": -118.2437, "location_name": "LA Distribution Center"},
        ...
        {"name": "Emergency Food Supplies", "resource_type": "Food", "description": "MRE packs for 500 people for 3 days", "quantity": 1500, "unit": "packs", "status": "available", "latitude": 29.7604, "longitude": -95.3698, "location_name": "Houston Depot"},
        {"name": "Communication Equipment", "resource_type": "Equipment", "description": "Satellite phones and portable radios", "quantity": 25, "unit": "sets", "status": "available", "latitude": 25.7617, "longitude": -80.1918, "location_name": "Miami Emergency Center"},
    ...
    # ── Disaster Events ──
    events = [
        {"event_type": "earthquake", "title": "Nepal Earthquake 2015", "description": "7.8 magnitude earthquake devastated Nepal.", "latitude": 28.2380, "longitude": 84.7314, "magnitude": 7.8, "affected_area_km2": 50000, "casualties": 8964, "injuries": 21952, "displaced": 3500000, "economic_damage_usd": 10000000000, "source": "USGS"},
        {"event_type": "flood", "title": "Kerala Floods 2018", "description": "Worst flooding in Kerala in nearly a century.", "latitude": 10.8505, "longitude": 76.2711, "affected_area_km2": 38863, "casualties": 483, "injuries": 1200, "displaced": 1400000, "economic_damage_usd": 5700000000, "source": "India Met Department"},
        {"event_type": "hurricane", "title": "Hurricane Katrina 2005", "description": "Category 5 hurricane that devastated the Gulf Coast.", "latitude": 29.9511, "longitude": -90.0715, "magnitude": 5.0, "affected_area_km2": 233000, "casualties": 1833, "injuries": 5000, "displaced": 1000000, "economic_damage_usd": 125000000000, "source": "NOAA"},
        {"event_type": "tsunami", "title": "Indian Ocean Tsunami 2004", "description": "Devastating tsunami triggered by 9.1 magnitude earthquake.", "latitude": 3.316, "longitude": 95.854, "magnitude": 9.1, "affected_area_km2": 500000, "casualties": 227898, "injuries": 125000, "displaced": 1700000, "economic_damage_usd": 15000000000, "source": "USGS"},
        {"event_type": "wildfire", "title": "Australian Black Summer 2019-2020", "description": "Catastrophic bushfire season across Australia.", "latitude": -33.8688, "longitude": 151.2093, "affected_area_km2": 186000, "casualties": 34, "injuries": 417, "displaced": 65000, "economic_damage_usd": 100000000000, "source": "Bureau of Meteorology"},
        {"event_type": "earthquake", "title": "Turkey-Syria Earthquake 2023", "description": "7.8 magnitude earthquake causing massive devastation.", "latitude": 37.174, "longitude": 37.032, "magnitude": 7.8, "affected_area_km2": 350000, "casualties": 59259, "injuries": 121000, "displaced": 9100000, "economic_damage_usd": 34000000000, "source": "USGS"},
    ]
    supabase.table(DISASTER_EVENTS_TABLE).insert(events).execute()

    print("✅ In-memory database seeded with demo data!")
    print("   📧 admin@disastermgmt.com / admin123")
    print("   📧 responder@disastermgmt.com / responder123")
    print("   📧 citizen@disastermgmt.com / citizen123")
...
async def startup_event():
    from database import is_fallback_mode
    print("\n🚀 Disaster Management System API started!")
    print(f"📖 API Docs: http://localhost:{settings.PORT}/docs")

    if is_fallback_mode():
        print("🗄️  Database: In-Memory Fallback (Supabase unavailable)")
        _seed_fallback_data()
    else:
        print(f"🗄️  Database: Supabase ({settings.SUPABASE_URL})")
